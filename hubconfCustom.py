...
def plot_one_boxCustom(x, img, color=None, label=None, line_thickness=3):
    ...
    # c2 = c1[0] + sizeLogo, c1[1] - sizeLogo - 3
        ...
        if ret:
          ...
          for i, det in enumerate(pred):
            ...
            if len(det):
              ...
              for *xyxy, conf, cls in reversed(det):
                ...
                # print(names[int(cls)],1)
                if names[int(cls)].strip() == 'person':
                  detectionTracker.append([xyxy[0],xyxy[1],xyxy[2],xyxy[3],label])
                else:
                  equipmentList.append([xyxy[0],xyxy[1],xyxy[2],xyxy[3],label])
              # print(detectionTracker)
          ...
          for box_id in boxes_ids:
            ...
            if frameCounter % 17 == 0:
              labelPlot = label_
            plot_one_boxCustom([int(x),int(y),int(w),int(h)],img0, label=labelPlot, color=colors[int(0)],line_thickness=3)
          frameCounter += 1
